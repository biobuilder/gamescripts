...
LOOK_DISTANCE = 10;
LOOK_MAX_DISTANCE = [40,40,40,40,30,30,30,30]
...
# randomization
RAND_DIR_CHANCE = 5 # chance of changing direction
...
def randomizeDirection( curDir, preDir, wallDetection ):
    ...
    if(curDir >= 0):
        if( WALL_PRESENT == wallDetection[curDir] or chance < RAND_DIR_CHANCE ):
            ...
            for i in range(1,10,1):
                ...
                if(wallDetection[r] == NO_WALL_PRESENT and preDir != REVERSE):
                    if(abs(int(wallDetection[r]) - int(r)) == 2):
                        print('det dir = ' + str(REVERSE))
                        return REVERSE
                    else:
                        print('dir = ' + str(r))
                        return r
...
while(esc_triggered == False):
    if( botState == STATE_READY ):
        ...
        direction = randomizeDirection(direction, prevDirection, wallStatus)
