...
POLE_SLOT_END = 2  # ending fishing pole slot (must be >= 1 and <= 9)
POLE_MAX_USES = 400  # how many uses (casts) a pole has before it switches
...
PICTURE_X = 1000  # screenshot top left corner x screen coordinate
...
PICTURE_W = 500  # width of the screenshot
PICTURE_H = 550
...
MOUSE_COOLDOWN = 2  # number of seconds to wait between successive clicks
CHECK_COOLDOWN = 0.1  # number of seconds to wait between checking the screen
...
mouse1 = pynput.mouse.Controller()
...
def on_press(key):
    ...
    try:
        if (key.char == ESCAPE_CHAR):
            esc_triggered = True
        if (key.char == SCREENSHOT_CHAR):
            take_screenshot = True
        if (key.char == FISHING_CHAR):
...
while (esc_triggered == False):
    ...
    if (state == SCREENSHOT_STATE):
        if (take_screenshot == True):
            ...
            keyboard1.press(str(pole_slot))
            keyboard1.release(str(pole_slot))
        else:
    ...
    if (state == FISHING_STATE):
        ...
        if (fishing_triggered == True):
            ...
            for pixels in pix_val:
                ...
                if ((pixels[0] > RED_THRESHOLD[0]) and (pixels[1] < RED_THRESHOLD[1]) and (
                        pixels[2] < RED_THRESHOLD[2])):
                    # store the best one
                    testPixel = pixels
            ...
            if (bestPixel[0] == 0):
                ...
                mouse1.press(pynput.mouse.Button.right)
                mouse1.release(pynput.mouse.Button.right)
